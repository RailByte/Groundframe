...
namespace GroundFrame.Core.UnitTests.TimeTable.WTTTimeTable
{
    ...
    public class WTTActivity
    {
        ...
        public void WTTActivity_Constructor_XElement()
        {
            ...
            GroundFrame.Core.Timetables.WTTActivity TestActivity  = new Timetables.WTTActivity(XMLTestActivity);

            //Run tests
            Assert.Equal((WTTActivityType)Convert.ToInt32(XMLTestActivity.Element("Activity").Value), TestActivity.ActivityType);
            Assert.Equal(XMLTestActivity.Element("AssociatedTrain")?.Value, TestActivity.AssociatedTrainHeadCode);
            Assert.Equal(XMLTestActivity.Element("AssociatedUID")?.Value, TestActivity.AssociatedUID);
        }
        ...
        public void WTTActivity_Constructor_JSON()
        {
            ...
            GroundFrame.Core.Timetables.WTTActivity TestJSONActivity = new Timetables.WTTActivity(JSON);

            //Run tests
            Assert.Equal((WTTActivityType)Convert.ToInt32(XMLTestActivity.Element("Activity").Value), TestJSONActivity.ActivityType);
            Assert.Equal(XMLTestActivity.Element("AssociatedTrain")?.Value, TestJSONActivity.AssociatedTrainHeadCode);
            Assert.Equal(XMLTestActivity.Element("AssociatedUID")?.Value, TestActivity.AssociatedUID);
